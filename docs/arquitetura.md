...
- **Verificação facial com documento por provedor externo** (`identity_verifications`, flag `identity_verification`; decisão do proprietário em 2026-09-21, que promove o item "liveness / face match" do backlog §4 do roadmap): quem compara a foto tirada na hora com a foto do documento é o **provedor** (Verifiky). A plataforma envia as imagens da captura e registra a resposta; **ela própria não compara rostos nem lê documento**. Interface, trilha e evidências só repetem o que o provedor informou, com o nome dele ("Verifiky informou: aprovado"). Resultado inconclusivo, erro ou tempo esgotado nunca libera o aceite. O vocabulário proibido (T1) continua valendo: o produto não afirma "identidade verificada" — afirma que um provedor nomeado comparou duas imagens e o que ele respondeu. Ver `docs/fase-4/verificacao-facial.md`.
...
Contratos Fase 1: `EmailProvider`, `PdfConverter`, `PdfSigner`, `PaymentGateway`. Implementações: `LaravelMailEmailProvider` (SMTP/HTTP do serviço próprio via mailer), `LogEmailProvider` (fake dev, identificado), `LibreOfficeConverter`, `ImageToPdfConverter`, `FakePdfConverter`, `PyHankoSigner`, `NullPdfSigner`, `MercadoPagoGateway`, `FakePaymentGateway`. Contratos reservados (interfaces documentadas, sem implementação): `SmsProvider`, `WhatsAppProvider`, `CpfVerificationProvider`, `CnpjLookupProvider`, `TimestampProvider`, `FiscalInvoiceProvider`. `IdentityVerificationProvider` deixou de ser reservado na Fase 4 §4.1: `VerifikyIdentityVerificationProvider` (real), `FakeIdentityVerificationProvider` (simulador identificado) e `DisabledIdentityVerificationProvider` (padrão) — `docs/integracoes/verifiky.md`. Cada adaptador: config própria, timeout, autenticação, tratamento de erro, `correlation_id`, política de repetição idempotente; resposta inconclusiva ≠ sucesso.
