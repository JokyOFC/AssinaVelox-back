    ...
    return (
        <>
            ...
            {subscription.status === 'past_due' && (
                <div className="border-warning-border bg-warning-bg text-warning flex flex-wrap items-center justify-between gap-3 rounded-[10px] border p-3.5 text-[13px]">
                    <span className="font-semibold">
                        Pagamento em atraso — regularize para continuar enviando
                        documentos.
                    </span>
                    {can.manage && (
                        <Button
            ...
            {pending_checkout && (
                <div className="border-primary-soft-border bg-primary-soft text-primary rounded-[10px] border p-3.5 text-[13px] font-medium">
                    Aguardando confirmação do pagamento pelo Mercado Pago. Isso
            ...
            >
                <div className="bg-navy flex flex-col gap-3 rounded-xl p-5 text-white">
                    ...
                    <div>
                        <div className="text-[26px] leading-none font-extrabold uppercase italic">
                            {plan.name}
                        </div>
                        <div className="text-on-navy-secondary mt-1.5 text-[13.5px]">
                            {plan.price_cents_monthly === 0
                                ? 'Grátis'
                                : `${formatCurrencyCompact(plan.price_cents_monthly)}/mês`}
                            {subscription.current_period_end &&
                                ` · renova em ${formatDayMonth(subscription.current_period_end)}`}
                        </div>
                    </div>
            ...
            >
                <div className="border-border bg-card shadow-card flex flex-col gap-3 rounded-xl border p-5">
                    <Heading
                        ...
                        description="Método do último pagamento aprovado (Mercado Pago)."
                    />
                    {payment_method ? (
                        <div className="flex items-center gap-3">
                            <span className="bg-navy flex h-[30px] w-11 items-center justify-center rounded-md text-white">
                            ...
                            <span>
                                ...
                                <span className="text-muted-foreground block text-[12.5px]">
                                    Cobrado via Checkout Pro a cada ciclo.
                                </span>
                            </span>
                        </div>
                    ) : (
                        <p className="text-muted-foreground text-[13px]">
                            Nenhum pagamento realizado ainda. Cartão, Pix e
                    ...
                    {billing_profile ? (
                        <p className="text-text-secondary text-[13px] leading-[1.7]">
                            <b className="text-foreground">
                                {billing_profile.legal_name}
                            </b>
                            <br />
                            CNPJ/CPF {billing_profile.document_number}
                            <br />
                            {billing_profile.address_line} ·{' '}
                            {billing_profile.city}/{billing_profile.state} ·{' '}
            ...
            <div className="border-border bg-card shadow-card rounded-xl border">
                <div className="flex flex-wrap items-center justify-between gap-3 px-5 pt-[18px] pb-3">
                    <Heading
                        variant="small"
                        title="Pagamentos"
                        description="Recibos internos em PDF. Nota fiscal chega na Fase 2."
                    />
                </div>
                <DataTable
                    columns={columns}
