...
| Planos (catálogo) | GET · POST · PATCH | `/admin/planos`, `/admin/planos/{plan}` | `admin.plans.index` · `admin.plans.store` · `admin.plans.update` | platform-admin (mutações: password.confirm) | `pages/admin/plans/index.tsx` | `Admin\PlanController` | 4 | Catálogo de planos: preço, cotas, recursos e visibilidade (docs/cobranca.md §18). Trilha em `platform_audit_events`. |
