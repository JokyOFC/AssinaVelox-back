...
- **Público-alvo declarado.** O serviço de integração é para "Órgãos da administração pública, autárquica e fundacional e Empresas Públicas, da União, Estados e Municípios". Servidores municipais precisam antes da adesão do município à Rede Nacional de Governo Digital. Fonte: página do _Serviço de integração_ (§2).
...
- **Envelopada (PAdES):** o integrador **prepara** o PDF (atualização incremental com placeholder e `/ByteRange`, ISO 32000-1). Em seguida calcula o SHA-256 dos _byte ranges_, envia via `assinarPKCS7` e embute o PKCS#7 devolvido. O manual recomenda que a primeira assinatura use DocMDP com `P=2`, para permitir novas assinaturas e preenchimento de formulário.
- **Exemplos oficiais:**
...
3. **Identidade.** O certificado do signatário precisa corresponder ao participante: nome e CPF. O formato do _subject_ dos certificados gov.br (onde fica o CPF) é **NÃO CONFIRMADO**. Ler de fixture real antes de codificar.
...
- Formato do _subject_ do certificado gov.br (onde aparecem nome e CPF).
