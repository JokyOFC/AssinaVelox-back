...
if (! function_exists('affiliateWithApproved')) {
    /**
...
if (! function_exists('affiliateAdminPost')) {
    /**
     * POST da equipe da plataforma, com ou sem senha confirmada.
     *
     * @param  array<string, mixed>  $parameters
     * @param  array<string, mixed>  $data
     */
    function affiliateAdminPost(User $admin, string $route, array $parameters = [], array $data = [], bool $confirmed = true): TestResponse
    {
...
test('o lote agrupa por afiliado só o que pode ser repassado; o resto fica para o próximo', function () {
    ...
    affiliateAdminPost($this->admin, 'admin.affiliates.payouts.store', data: ['currency' => 'BRL'])->assertRedirect();
...
test('saldo líquido negativo ou abaixo do mínimo não entra; estorno negativo abate no lote seguinte', function () {
    ...
    affiliateAdminPost($this->admin, 'admin.affiliates.payouts.store', data: ['currency' => 'BRL'])->assertSessionHasErrors('cutoff');
...
test('marcar como pago é manual, exige senha confirmada e registra quem, quando e referência', function () {
    ...
    affiliateAdminPost($this->admin, 'admin.affiliates.payouts.store', data: ['currency' => 'BRL']);
    $batch = PayoutBatch::query()->sole();

    // Sem senha confirmada: vai para a confirmação e nada muda.
    affiliateAdminPost($this->admin, 'admin.affiliates.payouts.paid', ['batch' => $batch->ulid], [
    ...
    affiliateAdminPost($this->admin, 'admin.affiliates.payouts.cancel', ['batch' => $batch->ulid], ['reason' => 'Tentativa indevida.'])
        ->assertSessionHasErrors('batch');
});

test('quem tem comissão no lote não registra o próprio pagamento; outra pessoa da equipe registra', function () {
    affiliateWithApproved(1, ['user_id' => $this->admin->id]);
    affiliateAdminPost($this->admin, 'admin.affiliates.payouts.store', data: ['currency' => 'BRL'])->assertRedirect();
    $batch = PayoutBatch::query()->sole();
    ...
    expect($batch->fresh()->status)->toBe(PayoutBatch::STATUS_DRAFT)
        ->and(Commission::query()->sole()->status)->toBe(Commission::STATUS_APPROVED)
        ->and(DB::table('affiliate_events')->where('action', 'payout_batch.paid')->count())->toBe(0);

    $colleague = User::factory()->platformAdmin()->create();

    affiliateAdminPost($colleague, 'admin.affiliates.payouts.paid', ['batch' => $batch->ulid], [
        'paid_at' => now()->toDateString(),
        'external_reference' => 'PIX E2E 1234',
    ])->assertRedirect()->assertSessionHas('success');

    expect($batch->fresh()->status)->toBe(PayoutBatch::STATUS_PAID)
        ->and($batch->fresh()->paid_by_user_id)->toBe($colleague->id);
});
...
test('cancelar um lote aberto devolve os lançamentos para o próximo', function () {
    ...
    affiliateAdminPost($this->admin, 'admin.affiliates.payouts.cancel', ['batch' => $batch->ulid], ['reason' => 'Valores revistos.'])
...
test('somente a equipe da plataforma acessa os lotes', function () {
    ...
    affiliateAdminPost($user, 'admin.affiliates.payouts.store', data: ['currency' => 'BRL'])->assertForbidden();
});
